...
class PayVat:
    ...
    def __init__(self):
        ...
        self._vat_list = tkinter.Listbox(
            self._window, selectmode=tkinter.MULTIPLE, font=default_font()
        )
        self._refresh()
        self._vat_list.place(
    ...
    def _refresh(self):
        ...
        for vat in payment.get_open_vat_payments():
            ...
            self._vat_list.insert(
                vat_count, vat.description + " " + astr + curr + " {" + vat.guid + "}"
            )
            vat_count += 1
        self._window.update()
